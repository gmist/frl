	...
	namespace console_std
	{
		// Переопределяем переменные
			...
			namespace backgroundColor
			{
				...
				const static DWORD default = blue & green & red;
			};
			...
			namespace foregroundColor
			{
				...
				const static DWORD iDefault = default | intensified;
			};
		#endif
		...
		public:
			// Constructors
			ColorOut();
			ColorOut( frl::Long foreground, frl::Long backgraund = backgroundColor::default );
			// Destructor
			~ColorOut();
			// override operator <<
			template < typename T >
