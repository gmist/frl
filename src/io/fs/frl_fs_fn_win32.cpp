	...
	namespace io
	{
		namespace fs
		{
			...
			FileRWCount write( FileDescriptor &file, const void *data, FileRWCount count )
			{
				...
				if ( count < 0 )
					FRL_THROW( FRL_STR( "Invalid count." ) );
				if ( count == 0 )
					return 0;
				FileRWCount writeCount = -1;
			...
			void concatenate( const String &fileName1, const String &fileName2, const String &fileNameDst, Bool overwrite )
			{
				...
				while( True )
				{
					...
					if( countRead < 0 )
						FRL_THROW( FRL_STR("Error on read file " ) + fileName1 );
					...
					if( countWrite < 0 )
						FRL_THROW( FRL_STR("Error on write file " ) + fileNameDst );
				}
				while( True )
				{
					countRead = read( dm.file2, buffer, sizeof(buffer) );
					if( countRead < 0 )
						FRL_THROW( FRL_STR("Error on read file " ) + fileName2 );
			...
			{	
				...
				SHFILEINFO sfi;
					...
					struct TimeUtil
					{
						...
						{
							return (time_-116444736000000000L)*100L;
						}
					};
