...
namespace frl
{
	namespace opc
	{
		...
		{
			version = lexicalCast<double>::get(newVersion);
		}
		...
		void ServerKind::registrerServer( const IID &implementCat )
		{
			...
			key.reinit(	FRL_STR("CLSID\\") +
				...
				lexicalCast<GUID, String >::get( implementCat ), key.getRootKey() );
			key.create();
